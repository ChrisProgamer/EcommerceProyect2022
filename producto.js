...
let recolectar_datos = function () {
  ...
  if (localStorage.getItem("miProducto")) {
    ...
    console.log(
      "Tomamos lo que este en el local storage y lo ponemos en el array: "
    );
    console.log(listado);
  }

  if (localStorage.getItem("miProducto")) {
    // Esta definido en el localStorage. Ya tiene productos
...
let probamosLocalStorage = function () {
  localStorage.setItem(
    "precio",
    document.querySelector(".act-price").innerText
  );
  localStorage.setItem("nombre", document.querySelector("#nombre").innerText);
  localStorage.setItem(
    "descuento",
    document.querySelector(".dis-price").innerText
  );
};
